...
type SafeResult = { rows: any[]; error: string | null };

async function safeQuery(label: string, sql: string): Promise<SafeResult> {
  try {
...
export default async function AdminDashboard() {
  ...
  const b: any = pay.rows[0] || {};
  const failedSections = [
    stats.error ? '營運統計' : null,
    cats.error ? '服務分類' : null,
    recent.error ? '最近服務者' : null,
    pay.error ? '收費狀態' : null,
  ...
  return (
    <div>
      <AdminPageHead
        ...
        title="平台營運總覽"
        description="帳號、申請、店家、收費與風險分流管理。"
        actions={
          <Link className="btn btn-brand" href="/admin/applications">
            處理待審核 <ArrowRight size={15} />
          </Link>
        }
      />
        ...
        <section className="admin-panel" style={{ marginBottom: 16 }}>
          <div className="admin-section-title">
            <div>
              <h3>部分資料暫時無法讀取</h3>
              <p>
                後台已進入保護模式，不會因單一資料表問題整頁白屏。異常區塊：
                {failedSections.join('、')}。
              </p>
            </div>
            <Link href="/api/admin/diagnostics">查看診斷</Link>
          </div>
        </section>
      )}

      <div className="admin-metric-grid">
        <AdminMetric label="顧客帳號" value={s.customers || 0} detail="顧客永久免費" />
        <AdminMetric label="服務者帳號" value={s.providers || 0} detail="個人＋店家成員" />
        <AdminMetric label="店家／工作室" value={s.organizations || 0} detail="以組織為單位管理" />
        <AdminMetric label="待審核申請" value={s.pending || 0} detail="集中申請池處理" />
        <AdminMetric label="今日需求" value={s.today_demands || 0} />
        <AdminMetric label="今日成交" value={s.today_bookings || 0} />
        <AdminMetric label="待處理檢舉" value={s.open_reports || 0} />
      </div>
      ...
      <div className="admin-grid-2">
        <section className="admin-panel">
          <div className="admin-section-title">
            ...
            <Link href="/admin/categories">管理分類</Link>
          </div>
          <div className="admin-category-grid">
            {cats.rows.length === 0 ? (
              <p className="muted">目前沒有可顯示的分類資料。</p>
            ) : (
              cats.rows.map((c: any) => (
                <div className="admin-category-card" key={c.key}>
                  <span>{c.label}</span>
                  <strong>{c.providers}</strong>
                  <small>已加入服務者 · {c.is_active ? '開放' : '停用'}</small>
                </div>
              ))
            )}
          </div>
        ...
        <section className="admin-panel">
          <div className="admin-section-title">
            ...
            <Link href="/admin/billing">方案與權限</Link>
          </div>
          <div className="admin-kpi-list">
            <div><span>已付款</span><strong>{b.active || 0}</strong></div>
            <div><span>試用中</span><strong>{b.trial || 0}</strong></div>
            <div><span>待付款</span><strong>{b.past_due || 0}</strong></div>
            <div><span>永久免費／合作</span><strong>{b.comped || 0}</strong></div>
          </div>
        </section>
      </div>
      ...
      <section className="admin-panel">
        <div className="admin-section-title">
          <div>
            <h3>最近加入的平台服務者</h3>
            <p>服務者註冊後進申請池，審核通過才進公開名錄。</p>
          </div>
          <Link href="/admin/providers">全部服務者</Link>
        </div>
        <div className="admin-table-wrap">
          <table className="admin-table">
            <thead>
              <tr><th>申請人</th><th>類別</th><th>店家／工作室</th><th>狀態</th><th>送出時間</th></tr>
            </thead>
            <tbody>
              {recent.rows.length === 0 ? (
                <tr><td colSpan={5}>目前沒有可顯示的服務者申請。</td></tr>
              ) : (
                recent.rows.map((x: any) => (
                  <tr key={x.user_id}>
                    <td><strong>{x.display_name}</strong><small>{x.email}</small></td>
                    <td><div className="tags">{(x.category_labels || []).map((c: string) => <span className="tag" key={c}>{c}</span>)}</div></td>
                    <td>{x.organization_name}</td>
                    <td><span className="tag">{x.application_status}</span></td>
                    <td>{x.submitted_at ? new Date(x.submitted_at).toLocaleString('zh-TW') : '尚未送審'}</td>
                  </tr>
                ))
              )}
            </tbody>
