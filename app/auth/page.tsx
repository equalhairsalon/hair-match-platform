...
function AuthContent(){
  ...
  const roleHint=qs.get('role')==='designer'?'designer':'customer';
  ...
  useMemo(()=>role==='designer'?(next==='/'?'/pro/onboarding':next):next,[role,next]);
  ...
  async function submit(e:FormEvent){
    ...
    try{
      ...
      if(mode==='register' && userRole==='designer') router.push('/pro/onboarding');
      else if(userRole==='designer' && next==='/') router.push('/pro/dashboard');
...
function AuthLoading(){
  return <div className="app-shell"><main className="container page"><div className="panel auth-card"><div className="muted">登入頁載入中…</div></div></main></div>;
}

export default function AuthPage(){
  return <Suspense fallback={<AuthLoading/>}><AuthContent/></Suspense>;
}
